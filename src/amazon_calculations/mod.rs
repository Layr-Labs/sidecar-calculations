...
#[no_mangle]
pub extern "C" fn amazon_staker_token_rewards_c(sp: *const c_char, tpd: *const c_char) -> *mut c_char {
    let sp_str = unsafe {
        assert!(!sp.is_null());
    ...
    let result = amazon_staker_token_rewards(sp_str, tpd_str);
...
#[no_mangle]
pub extern "C" fn amazon_operator_token_rewards_c(tsot: *const c_char) -> *mut c_char {
    let tsot_str = unsafe {
        assert!(!tsot.is_null());
        CStr::from_ptr(tsot).to_str().unwrap()
    };

    let result = amazon_operator_token_rewards(tsot_str);
